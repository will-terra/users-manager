...
const percent =
  totalStatements === 0 ? 0 : (totalCovered / totalStatements) * 100;

console.log("Coverage summary:");
console.log(`  Frontend: ${fe.covered}/${fe.total}`);
console.log(`  Backend:  ${be.covered}/${be.total}`);
console.log(
  `  Combined: ${totalCovered}/${totalStatements} -> ${pretty(percent)}%`,
);

if (percent < threshold) {
  console.error(
    `Coverage ${pretty(percent)}% is below the threshold of ${threshold}%`,
  );
  process.exit(1);
}

console.log(
  `Coverage ${pretty(percent)}% meets the threshold of ${threshold}%`,
);
process.exit(0);
